...
class DataPartitioner {
  public:
    DataPartitioner();
    DataPartitioner(size_t K, std::vector<std::vector<std::pair<int, float>>>& w_index):
                    K_(K), w_index_(w_index) {}
    ~DataPartitioner();
    void Clear();
    ...
    float RunNeighbourExpansion(const std::vector<std::vector<std::pair<int, float> > > &data_vec,
                                const std::vector<std::vector<int> > &labels_vec,
    ...
    const std::vector<std::vector<std::pair<int, float> > >& w_index() const { return w_index_; }
