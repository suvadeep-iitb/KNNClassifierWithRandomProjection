...
class RandomEmbeddingAKNNPredictor(KNNPredictor):
  ...
  def Train(self, X, Y, numThreads):
    ...
    self.graph = self.CreateAKNNGraph(self.W, X, numThreads)
    self.Y = Y


  def LearnRandomEmbeddedRegression(X, Y, numThreads):
    L = self.labelDim
    D = self.featureDim
    embDim = self.embDim
    C = self.lamb
